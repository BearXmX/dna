                    ...
                    <template v-else-if="item.type === 'sugar' || item.type === 'sugar_reverse'">
                      <svg viewBox="-10 0 92 80" aria-hidden="true">
                        <g :class="{ 'sugar-preview-reverse': item.type === 'sugar_reverse' }">
                          <!-- v107：打印版组件库预览同步补 4→5 折线；正向为倒 V 且左侧边短一点，反向整体旋转后右侧边短一点。 -->
...
function createSceneBackgroundTexture() {
  ...
  // v107：打印版背景同步演示版，保留蓝色科技风，去掉暗色渐变并提亮，不使用接近白色的背景。
...
function createSugarDefaultFiveFoldVisual() {
  ...
  start.y += 0.03
  start.z = 0.28
  const corner = new THREE.Vector3(start.x - 0.18, start.y + 0.22, 0.28)
  const end = new THREE.Vector3(corner.x - 0.42, corner.y + 0.08, 0.28)
...
function buildBaseVisual(group: THREE.Group, config: PaletteItem) {
  ...
  // v92：移除碱基分子表面的 A/T/G/C 英文字母，只保留结构形态和颜色区分。
...
function getBaseSolidEdgeLocalPoint(base: THREE.Group, towardLocal: THREE.Vector3, lineIndex = 0, lineCount = 1) {
  const data = getMoleculeData(base)
  const normal = new THREE.Vector2(towardLocal.x, towardLocal.y)
  if (normal.lengthSq() < 0.0001) normal.set(towardLocal.x >= 0 ? 1 : -1, 0)
  normal.normalize()

  const tangent = new THREE.Vector2(-normal.y, normal.x)
  const offsets = lineCount >= 3 ? [-0.22, 0, 0.22] : lineCount === 2 ? [-0.16, 0.16] : [0]
  const offset = offsets[lineIndex] ?? 0

  let ringCenter = new THREE.Vector2(0, 0)
  let outerRadius = 1.02
  let innerRadius = outerRadius * 0.58

  if (data.type === 'A' || data.type === 'G') {
    // v93：嘌呤双环必须选其中一个实体环的边缘，不能把锚点放到两个环的镂空中心。
    const useRightRing = Math.abs(normal.x) > 0.12 ? normal.x >= 0 : towardLocal.x >= 0
    ringCenter = useRightRing ? new THREE.Vector2(0.58, 0) : new THREE.Vector2(-0.45, 0)
    outerRadius = useRightRing ? 0.78 : 0.86
    innerRadius = outerRadius * 0.58
  }

  const radial = innerRadius + (outerRadius - innerRadius) * 0.58
  const p = ringCenter
    .clone()
    .add(normal.clone().multiplyScalar(radial))
    .add(tangent.clone().multiplyScalar(offset))

  // 防止偏移后滑进镂空区或者滑出外边缘：始终压回实体环带内部。
  const fromCenter = p.clone().sub(ringCenter)
  const distance = fromCenter.length()
  const minDistance = innerRadius + 0.075
  const maxDistance = outerRadius - 0.07
  if (distance < minDistance) fromCenter.setLength(minDistance)
  if (distance > maxDistance) fromCenter.setLength(maxDistance)
  const safe = ringCenter.clone().add(fromCenter)

  return new THREE.Vector3(safe.x, safe.y, 0.12)
}
...
function getBaseHydrogenAnchorSeriesRootLocal(base: THREE.Group, toward: THREE.Vector3, count: number) {
  ...
  if (!root) return [center.add(dirRoot.normalize().multiplyScalar(getBaseConnectRadius(base) * 0.86))]
  ...
  // v93：每一根 A-T / G-C 红线都吸附在“实体环带边缘”上，
  // 不再从碱基镂空中心穿过去，也不再外漏到边缘外面。
  return Array.from({ length: count }, (_, index) => {
    const localPoint = getBaseSolidEdgeLocalPoint(base, towardLocal, index, count)
...
function createHydrogenBondDots(group: THREE.Group, a: THREE.Group, b: THREE.Group, count: number) {
  ...
  // v94：A-T / G-C 红线绝对不能交叉。
  // 两端锚点先按同一条公共方向排序，再一一连接，保证低对低、高对高。
  const pairDir = b.position.clone().sub(a.position)
  if (pairDir.lengthSq() < 0.0001) pairDir.set(1, 0, 0)
  pairDir.normalize()
  const preferredAxis = new THREE.Vector3(0, 1, 0)
  const sortAxis = preferredAxis.sub(pairDir.clone().multiplyScalar(preferredAxis.dot(pairDir)))
  if (sortAxis.lengthSq() < 0.0001) sortAxis.set(-pairDir.y, pairDir.x, 0)
  if (sortAxis.lengthSq() < 0.0001) sortAxis.set(0, 1, 0)
  sortAxis.normalize()

  const sortedA = [...aAnchors].sort((p1, p2) => p1.dot(sortAxis) - p2.dot(sortAxis))
  const sortedB = [...bAnchors].sort((p1, p2) => p1.dot(sortAxis) - p2.dot(sortAxis))

  for (let lineIndex = 0; lineIndex < lines; lineIndex++) {
    const radius = count === 3 && lineIndex === 1 ? 0.048 : 0.068
    const s = sortedA[lineIndex]
    const e = sortedB[lineIndex]
    // 锚点已经在实体环带内部，不再外延，避免红线穿孔或漏出。
    group.add(createCylinderBetweenWithEndpointOverlap(s, e, radius, color, 1, false, 0, 0))
  }
}

function createCylinderBetween(start: THREE.Vector3, end: THREE.Vector3, radius: number, color: number, opacity: number, additive = false, overlap = 0.025) {
  const direction = end.clone().sub(start)
  const rawLength = direction.length()
  if (rawLength < 0.0001) return new THREE.Group()

  // v92：连接点已经放在分子实体内部，所以这里不再按线半径强制外延。
  // 只保留极小嵌入，避免线段露出边缘或穿出另一侧。
  const unit = direction.clone().normalize()
  const safeOverlap = Math.min(Math.max(overlap, 0), rawLength * 0.045)
...
function createCylinderBetweenWithEndpointOverlap(
  ...
  const unit = direction.clone().normalize()
  // v92：折线端点只允许轻微嵌入，不再大幅外延，避免圆柱线漏出分子边缘。
  const maxOverlap = rawLength * 0.08
...
function getBaseSurfaceAnchorRootLocal(base: THREE.Group, toward: THREE.Vector3) {
  ...
  const dirRoot = toward.clone().sub(center)
  if (dirRoot.lengthSq() < 0.0001) return center
  if (!root) return center.add(dirRoot.normalize().multiplyScalar(getBaseConnectRadius(base) * 0.82))
  ...
  // v93：糖-碱基灰线也必须接在碱基实体环带边缘，不能穿过镂空中心。
  const localPoint = getBaseSolidEdgeLocalPoint(base, towardLocal, 0, 1)
  localPoint.z = 0.1
  return toRootLocalPoint(base.localToWorld(localPoint))
}
...
function getSugarBaseAnchorWorld(sugar: THREE.Group) {
  // v92：糖-碱基连接点继续向糖环内部回收，线头完全卡在糖环实体里，不再露出边缘。
  sugar.updateWorldMatrix(true, false)
...
function createGlycosidicTubeForBond(bond: BondData, radius: number, color: number, opacity: number, additive = false) {
  ...
  const group = new THREE.Group()
  group.add(createCylinderBetween(start, end, radius, color, opacity, additive, 0))
  return group
}
...
function createSingleFoldTubeBetween(start: THREE.Vector3, end: THREE.Vector3, radius: number, color: number, opacity: number, foldSign = 1, additive = false, port: SugarBackbonePort = '3') {
  ...
  const endpointOverlap = Math.max(0.018, radius * 0.22)
  ...
  // v92：折线转折处增加实体圆点，遮住两段圆柱的拼接缝，避免出现“快断了”的断层感。
  const jointMaterial = new THREE.MeshBasicMaterial({ color, transparent: opacity < 0.999, opacity })
  const cornerJoint = new THREE.Mesh(new THREE.SphereGeometry(radius * 1.36, 18, 18), jointMaterial)
  cornerJoint.position.copy(corner)
  group.add(cornerJoint)

    // v93：折线两端也补小实体点，遮住圆柱拼接口的细小断层。
    ;[start, end].forEach((point) => {
      const endpointJoint = new THREE.Mesh(new THREE.SphereGeometry(radius * 0.92, 16, 16), jointMaterial.clone())
      endpointJoint.position.copy(point)
      group.add(endpointJoint)
    })
...
function createCustomHelixLayoutFromCurrentScene() {
  ...
  const pairGap = 5.55
  const leftSugarX = -5.15
  const rightSugarX = 5.15
  const leftBaseX = -1.08
  const rightBaseX = 1.08
  ...
  const getCustomHelixPoint = (pairIndex: number, side: number, role: 'sugar' | 'base' | 'phosphate') => {
    ...
    const angleStep = THREE.MathUtils.degToRad(totalPairs <= 3 ? 42 : 34)
    const phase = side >= 0 ? 0 : Math.PI
    const baseAngle = phase + centered * angleStep
    const rise = totalPairs <= 3 ? 6.2 : 3.65
    ...
    const radius = 6.9
    const angle = baseAngle - side * 0.26
  ...
  molecules.forEach((group) => {
    ...
    if (data.type === 'phosphate') {
      ...
      if (connectedSugarItems.length) {
        ...
        const planeX = side > 0 ? leftSugarX - 1.42 : rightSugarX + 1.42
...
function loadPresetDnaStructure() {
  ...
  // v87：完整科学结构只保留半段，减少课堂展示时的拥挤感。
  const seq: Array<'A' | 'T' | 'G' | 'C'> = ['A', 'G', 'T', 'C', 'A']
  ...
  const sugarRadius = 5.15
  const phosphateRadius = 6.9
  const baseRadius = 1.18
  ...
  for (let i = 0; i < seq.length - 1; i++) {
    ...
    const topPlane = new THREE.Vector3(leftSugarX - 1.42, midY, 0)
    const bottomPlane = new THREE.Vector3(rightSugarX + 1.42, midY, 0)
  ...
  const terminalOffset = gap * 0.18
  const terminalPlaneAndHelix = [
    { plane: new THREE.Vector3(leftSugarX - 1.42, startY + terminalOffset, 0), helix: getHelixMidPoint(-0.35, seq.length, phosphateRadius, helixHeight, 0), sugar: topSugars[0], rule: connectionRules['phosphate-sugar'], beforeSugar: true, sugarPort: '5' as SugarBackbonePort },
    { plane: new THREE.Vector3(leftSugarX - 1.42, -startY - terminalOffset, 0), helix: getHelixMidPoint(seq.length - 0.65, seq.length, phosphateRadius, helixHeight, 0), sugar: topSugars[topSugars.length - 1], rule: connectionRules['sugar-phosphate'], beforeSugar: false, sugarPort: '3' as SugarBackbonePort },
    { plane: new THREE.Vector3(rightSugarX + 1.42, startY + terminalOffset, 0), helix: getHelixMidPoint(-0.35, seq.length, phosphateRadius, helixHeight, Math.PI), sugar: bottomSugars[0], rule: connectionRules['phosphate-sugar'], beforeSugar: true, sugarPort: '3' as SugarBackbonePort },
    { plane: new THREE.Vector3(rightSugarX + 1.42, -startY - terminalOffset, 0), helix: getHelixMidPoint(seq.length - 0.65, seq.length, phosphateRadius, helixHeight, Math.PI), sugar: bottomSugars[bottomSugars.length - 1], rule: connectionRules['sugar-phosphate'], beforeSugar: false, sugarPort: '5' as SugarBackbonePort },
  ]
  ...
  resetCamera()
  updateStats()
  hintText.value = '已生成半段科学 DNA 结构：组数减少，连接线段更短，适合课堂近距离观察。'
